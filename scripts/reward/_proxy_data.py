...
from _sr2_direct import labels_complete_path, read_jsonl  # noqa: E402
...
def as_arrays(rows: Sequence[Mapping], arm: str = "a") -> Dict[str, np.ndarray]:
    key = f"features_{arm}"
    ...
    return {
        "features": np.asarray([r[key] for r in rows], dtype=np.float64),
